...
uint16_t LCD_ReadPoint(uint16_t x,uint16_t y)
{
	...
	if(lcddev.id==0X9325||lcddev.id==0X4535||lcddev.id==0X4531||lcddev.id==0XB505||lcddev.id==0XC505 ||lcddev.id==0X8999)return r;	//这几种IC直接返回颜色值
...
void TFTLCD_Init(void)
{
	...
	{	   
    	...
    	LCD_WriteReg(0x0001,0x3B3F);//驱动输出控制320*240  0x6B3F
	...
	//初始化完成以后,提速
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963||lcddev.id==0X8999)//如果是这几个IC,则设置WR时序为最快
	{
		//重新配置写时序控制寄存器的时序   	 							    
